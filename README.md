...
### `GqlSignalStatus` Component

A component that handles loading, error, and success states declaratively.

**Input:**
- `gql` (required): A `GqlSignalResult<T>` signal from `gqlQuery`

**Content Projection:**
- `[gqlLoading]`: Template shown during loading state
- `[gqlError]`: Template shown during error state
- Default slot: Content shown when data is available

### `provideGqlSignalConfig(config)`

Configures global defaults for loading and error templates.

**Parameters:**
- `config.loadingDefaultTemplate?: string | Type<unknown>` - Default loading template
- `config.errorDefaultTemplate?: string | Type<unknown>` - Default error template

**Returns:**
Angular `Provider` for use in application config
